...
def train_one_epoch1(model, optimizer, data_loader, device, epoch, args):
    for p in optimizer.param_groups:
        p["lr"] = args.lr_epoch
    ...
    for i, (image, target) in enumerate(data_loader):
        ...
        if (i+1) % batch_size != 0 and i != iters-1:
            if is_fst_loss:
                loss = total_loss
                is_fst_loss = False
            else:
...
def evaluate(model, data_loader, device, args, generate=True):
    ...
    import pickle
    with open('/output/result.pkl', 'wb') as aa:
        pickle.dump(results, aa)
    S = time.time()
