...
class YOLO(object):
    _defaults = {
        "model_path": '/data/zihaosh/hw2_load/10711.pth',
    ...
    def detect_image(self, image):
        ...
        crop_img = np.array(letterbox_image(image, (self.model_image_size[0],
                                                    self.model_image_size[1])))
        ...
        topindex = batch_detections[:, 4]*batch_detections[:, 5]
        top_index = topindex > self.confidence
        a = batch_detections[top_index, 4]
        b = batch_detections[top_index, 5]
        top_conf = a*b
        ...
        boxes = yolo_correct_boxes(top_ymin, top_xmin, top_ymax, top_xmax,
                                   np.array([self.model_image_size[0],
                                             self.model_image_size[1]]),
                                   image_shape)
        ...
        for i, c in enumerate(top_label):
            ...
            bottom = min(np.shape(image)[0],
                         np.floor(bottom + 0.5).astype('int32'))
            right = min(np.shape(image)[1],
