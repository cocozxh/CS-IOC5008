...
def fit_one_epoch(net, yolo_losses, epoch, epoch_size, epoch_size_val,
                  gen, genval, Epoch, cuda, optimizer, lr_scheduler):
    total_loss = 0
    val_loss = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        if iteration == 0 or (iteration + 1) % 10 == 0:
            print('step:' + str(iteration + 1) + '/' + str(epoch_size) +
...
for epoch in range(Init_Epoch, Freeze_Epoch):
    total_loss, val_loss = fit_one_epoch(model, yolo_losses, epoch, epoch_size, epoch_size_val, gen, gen_val,
                                         Freeze_Epoch, Cuda, optimizer, lr_scheduler)
    if total_loss < best_loss:
        best_loss = total_loss
        best_model_weights = copy.deepcopy(model.state_dict())
        torch.save(best_model_weights, '/output/weights' + str(epoch) + '.pth')
    with open('/output/total_loss.csv', mode='a+') as total_loss_file:
        total_loss_file.write(str(total_loss.item()) + '\n')
    with open('/output/val_loss.csv', mode='a+') as val_loss_file:
...
for epoch in range(Freeze_Epoch, Unfreeze_Epoch):
    ...
    with open('/output/val_loss.csv', mode='a+') as val_loss_file:
        val_loss_file.write(str(val_loss.item()) + '\n')
torch.save(best_model_weights, '/output/yolov4_maskdetect_weights1.pth')
