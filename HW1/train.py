...
val_dataset = torchvision.datasets.ImageFolder(val_save_path, transform=transforms_img)
...
# 模型定义-ResNet
model_ft = models.resnet50(pretrained=args.pre_train)
...
optimizer = optim.Adam(filter(lambda p: p.requires_grad, model_for_hw1.parameters()), 
        ...
        with open("/output/log.txt", "w")as f2:
            for epoch in range(args.epoch):
                ...
                with torch.no_grad():
                    ...
                    print('Validation accuracy is: %.3f%%' % (100 * correct / total))
                    acc = 100. * correct / total
                    # 将每次测试结果实时写入acc.txt文件中
                    print('Saving model......')
                    torch.save(model_for_hw1.state_dict(), '%s/net_%03d.pth' % ('/output', epoch + 1))   
